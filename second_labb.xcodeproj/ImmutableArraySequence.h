...
public:
    ...
    Sequence<T>* GetSubsequence(int start_index, int end_index) const override {
        if (start_index < 0 || start_index >= this->GetLength() || end_index < 0 || end_index >= this->GetLength() || start_index > end_index) {
    ...
    Sequence<T2>* Map(T2 (*mapper)(T)) const {
        ...
        for (int index = 0; index < this->GetLength(); index++) {
            Sequence<T2>* next = new_array->Append(mapper(this->items->Get(index)));
            delete new_array;
            new_array = static_cast<ImmutableArraySequence<T2>*>(next);
        }
        return new_array;
    }
    ...
    Sequence<T>* Where(bool (*where)(T)) const override {
        ...
        for (int index = 0; index < this->GetLength(); index++) {
            if (where(this->Get(index))) {
                ...
                delete new_array;
                new_array = static_cast<ImmutableArraySequence<T>*>(next);
            }
        }
        return new_array;
    }
